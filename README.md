...
| 方案 | 结果 |
|---|---|
| RSSHub 公开实例（微博/小红书/X 的 RSS 桥接） | X 重定向到 404、微博 403、小红书 403 |
| LinkedIn 公司页 | robots 拒绝 |
| 官方 blog RSS | Whering / Indyx / Style DNA / Acloset 都返回 200 但 0 条目（SPA 页面不是 RSS） |
| 官方 newsroom RSS | 只有 Stitch Fix 这类上市公司有，已接入 |
| 按创始人姓名搜新闻 | ✅ 可用，已接入 |

结论：原始发帖拿不到，除非上 X API（约 $200/月）。但真正影响格局的动作
（融资、重大合作、战略采访）一定会被媒体报道，`founder` 字段的新闻搜索能覆盖。
实测 Alta 创始人的 CFDA 合作、LVMH 投资、名人背书全都抓到了。
